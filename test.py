...
def dictmap(data, num=1):
    ...
    fnames = []
    for line in data:
        name = int(line.split('.')[0].split('/')[num])#比如'train/4325.jpg'先以'.'分割取第一个，然后以'/'分割取第二个得到4325
        fnames.append(name)
...
def default_loader(path):
    ...
    #Image.convert(‘RGB’)时,它只是将每个像素转换为三重8位值.因此,它从根本上改变了图像的表示和存储方式.
...
class mydataset(Dataset):
    ...
    def __init__(self,data,lab_dict,loader=default_loader,mode='train'):
        ...
        for line in data:
            la.append(lab_dict[int(line.split('.')[0].split('/')[1])])#比如'train/4325.jpg'先以'.'分割取第一个，然后以'/'分割取第二个得到4325
    ...
    def __getitem__(self,index):
        #fn是图片path #fn和label分别获得imgs[index]
        fn = self.data[index] #比如'train/4325.jpg'
        label =  self.la[index]#比如#比如4325
        img = self.loader(fn)#按照路径读取图片：比如根据'train/4325.jpg'获取图片
        if self.transforms is not None:
            img = self.transforms(img)#数据标签转换为Tensor
        return img,torch.from_numpy(np.array(label))#torch.from_numpy这个方法将numpy类转换成tensor类，
...
def Train_Testloader(data):
    ...
    test_loader = DataLoader(test_dataset,batch_size=1,shuffle=False,num_workers=10,pin_memory=True)
